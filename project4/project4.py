...
This function will return the random element in the adjList

INPUTS
adjList: the list you want to pick a random element from

OUTPUTS
adjList[pos]:   the random element in the adjList
"""
def randomVertex(adjList):
    pos = random.randint(0, len(adjList)-1)
    return adjList[pos]
...
This function will generate the MST from given graph using prim's algorithm
The function only updates the prev attribute in each vertex, which reflects a MST

INPUTS
adjList: the list containing all vertices in the graph
adjMat:  the adjMat which containing all weights between nodes
...
def prim(adjList, adjMat):
    ...
    # initialize all costs to inf and prev to null and visited to false
    for vertex in adjList:
        vertex.cost = math.inf
        vertex.prev = None
        vertex.visited = False

    # pick an arbitrary start vertex and set cost to 0, and set visited as true
    start = randomVertex(adjList)
    start.cost = 0
    Q = MinQueue(adjList)
...
This function generate the MST from the given graph using kruskal's algorithm.
The function only updates the prev attribute in each vertex, which reflects a MST
...
"""
TSP
note that although we got the MST, when we try to get the tour route, 
this maybe not the shortest path

This function will traverse the whole MST using DFS, return the traverse route.
Adding start vertex to the end of traverse route to generate a cycle, 
which is the whole path of traveling salesman problem.

INPUT
adjList: the list containing all vertices in the MST
start:   the start vertex

OUTPUT
tour:    the whole path of traveling salesman problem
"""
def tsp(adjList, start):

    # Initialize the empty route
    tour = []
    ...
    while len(stack) != 0:
        ...
        # Get the current vertex, mark it as visited, add it into traverse route
        current = stack.pop()
        current.visited = True
        tour.append(current.rank)

        # Push all unvisited neighbors into stack
        for neighbor in current.mstN:
            if neighbor.visited is False:
                stack.append(neighbor)

    # Add the start vertex to the route
...
OUTPUT
v.pi: the root of current vertex
"""
def find(v):
...
INPUT
u: the first vertex need to be union
v: the second vertex need to be union
